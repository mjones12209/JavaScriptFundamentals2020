...
const listenerOne =  async () => {
  ...
  observer.subscribe(listenerOne);

  document.querySelector("#currency2").addEventListener("change", (event) => {
    observer.notify();
  });
  document.querySelector("#currency1").addEventListener("change", (event) => {
    observer.notify();
  });
})();
